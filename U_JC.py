...
#    print("init",initial[0,2,0],initial[1,0,0],initial[0,0,1],initial[0,0,0], initial.shape)
	...
	#        print("env",env[0,0,0],env[1,0,0],env[0,0,1])
		...
		sys_env += (.5*(MB(sys_env)) -
					0.5j*dt*( Bd(Delc*c(initial)+Omc*initial+g*sm(initial)) + B(Delc*cd(initial)+Omc*initial+g*sp(initial))
						 - dt/3*(gamma_L+gamma_R)*( 2*Delc*nc(initial) + Omc*(c(initial)+cd(initial)) + g*(sp(c(initial))+cd(sm(initial))) )
						 + 2*Delc/3*Bd(B(initial)) ) )
