...
def env_dens(env):
	...
	# Indices of the timebins initially: 0->furthest past, L->system, L+1->first future timebin
	if len(env.shape)==1:
		dens = np.einsum("i,j->ij",np.conjugate(env),env)
	elif len(env.shape)==2:
		dens = np.einsum("ki,kj->ij",np.conjugate(env),env)
	else:
		dens = np.einsum("ilk,imk->lm",np.conjugate(env),env)
	return dens
...
#    first_state = np.einsum("ijk,lj->ilk",states[index],dB)
    corr[0] = np.einsum("mil,mil",np.conjugate(states[index]),np.einsum("ik,mkl->mil",np.einsum("ij,jk->ik",dBd,dB),states[index]))#-offset
#    offset    = np.einsum("ilk,ilk",first_state,np.conjugate(first_state))
    sum_B = corr[0]*np.ones(freqs.shape)
    next_step  = np.einsum("ilm,ilk->mk",np.conjugate(states[index]),np.einsum("lj,ijk->ilk",dBd,states[index]))#/corr[0]
...
#            print(next_step.shape, states[index-p].shape)
            corr[p] = 	(np.einsum("lk,lk",next_step,np.einsum("li,ki->lk",np.conjugate(states[index-p]),np.einsum("ij,kj->ki",dB,states[index-p]))))#-offset))
            sum_B     += (corr[p-1])*np.exp(1j*freqs*p*dt)
            next_step  = np.einsum("lk,lk",next_step,np.einsum("li,ki->lk",np.conjugate(states[index-p]),states[index-p]))
        elif len(states[index-p].shape)==3:
#            print(next_step.shape, states[index-p].shape)
            corr[p]     += (np.einsum("ji,ji",next_step,np.einsum("jlk,ilk->ji",np.conjugate(states[index-p]),np.einsum("lj,ijk->ilk",dB,states[index-p]))))
            sum_B     += (corr[p-1])*np.exp(1j*freqs*p*dt)
            next_step  = np.einsum("jmk,jml->kl",np.conjugate(states[index-p]),np.einsum("ji,imk->jmk",next_step,states[index-p]))
#    return 2./dt*np.real(sum_B)
    return 2./dt*np.real(sum_B),2/dt*np.imag(sum_B),corr
...
def g2_out(states,taumax,N_env,dt,index):
    ...
    tau = np.zeros(taumax)
    temp      = np.einsum("jl,ilk->ijk",np.einsum("jf,fl->jl",dB,dB),states[index])
    temp2    = np.einsum("jl,ilk->ijk",dB,states[index])
    denom    = np.einsum("ijk,ijk",np.conjugate(temp2),temp2)**2
    g2_out[0] = np.einsum("ijk,ijk", np.conjugate(temp),temp)/denom
    next_step = np.einsum("ijk,ijl->kl",np.conjugate(temp2),temp2)
    for it in range(1,taumax):
        tau[it] = dt*it
        if len(states[index-it].shape)==2:
            temp      = np.einsum("ij,kj->ki",dB,states[index-it])
            g2_out[it] = np.einsum("ji,ji",next_step,np.einsum("kj,ki->ji", np.conjugate(temp),temp))/denom
            next_step = np.einsum("ji,ji",next_step,np.einsum("kj,ki->ji",np.conjugate(states[index-it]),states[index-it]))
        elif len(states[index-it].shape)==3:
            temp      = np.einsum("lj,ijk->ilk",dB,states[index-it])
            g2_out[it] = np.einsum("mn,mn",next_step,np.einsum("mjk,njk->mn", np.conjugate(temp),temp))/denom
            next_step = np.einsum("ikj,ikl->jl",np.conjugate(states[index-it]),np.einsum("ij,jkl->ikl",next_step,states[index-it]))
